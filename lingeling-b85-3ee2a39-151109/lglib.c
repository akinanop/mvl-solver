...
typedef struct Stats {
  ...
  int defrags, reported, repcntdown, gcs, decomps;
  struct { int clauses, vars; } rescored;
  struct { int64_t count;
           struct { int64_t count, saturating, agile, reuse; } skipped;
  ...
  uint64_t avgrealglue, avgrealglueslow, avgrealgluefast;
  struct { uint64_t avg; int64_t der, last; } jlevel;
  struct { uint64_t avg; int64_t der; int count; } its;
  struct {
  ...
  struct {
    struct { int irr, red, bin, trn, lrg; } sub, str;
  ...
  struct { int count; int64_t trn, sub; } quatres;
...
typedef struct Limits {
  ...
  struct {
    ...
    int cinc, itinc, binc, tinc, pen; } simp;
...
struct LGL {
  ...
  Scr scinc, scincf, maxscore, minscore;
  int scincinc;
  ...
  Stk dsched, eassume, assume, learned;
#ifndef NCHKSOL
  Stk orig;
#endif
  Queue queue;
...
static void lglsetmaxminscore (LGL * lgl) {
  Scr oldmaxscore = lgl->maxscore, oldminscore = lgl->minscore;
  lgl->maxscore = lglflt (lgl->opts->maxscorexp.val, 1);
  lgl->minscore = lglflt (lgl->opts->minscorexp.val, 1);
  if (oldmaxscore != lgl->maxscore)
    lglprt (lgl, 1,
      "[set-maximum-score] maximum score limit %s (--maxscorexp=%d)",
      lglscr2str (lgl, lgl->maxscore), lgl->opts->maxscorexp.val);
  if (oldminscore != lgl->minscore)
    lglprt (lgl, 1,
      "[set-minimum-score] minimum score limit %s (--minscorexp=%d)",
      lglscr2str (lgl, lgl->minscore), lgl->opts->minscorexp.val);
}
...
static int lglscrcmp (LGL * lgl, Scr a, Scr b) {
...
static void lglenq (LGL * lgl, int idx, int updatequeuenext) {
  ...
  if (updatequeuenext ||
      lgl->queue.next < 0 ||
      (!lglval (lgl, idx) && lgl->opts->queuedfs.val))
...
static void lglrescorevars (LGL * lgl) {
  ...
  for (idx = 2; idx < lgl->nvars; idx++) {
	...
	assert (!qv->enqueued);
	if (lgl->opts->queueunsched.val) {
	  if (qv->pos >= 0) lgldunsched (lgl, idx);
	  lglenq (lgl, idx, 0);
	}
      }
    }
...
static void lglunassign (LGL * lgl, int lit) {
  ...
  if (qv->enqueued) {
    assert (qv->pos >= 0);
    if (lgl->opts->queuedfs.val && lgl->queue.next < qv->pos)
      lgl->queue.next = qv->pos;
...
static double lglheight (LGL * lgl) {
  return lgl->stats->jlevel.avg / (double)(1ull<<32);
}

static double lglglue (LGL * lgl) {
  return lgl->stats->avgrealglue / (double)(1ull<<32);
}

static double lglglueslow (LGL * lgl) {
  return lgl->stats->avgrealglueslow / (double)(1ull<<32);
}

static double lglgluefast (LGL * lgl) {
  return lgl->stats->avgrealgluefast / (double)(1ull<<32);
}
...
static void lglrephead (LGL * lgl) {
  ...
  fprintf (lgl->out, "%s\n", lgl->prefix);
  fprintf (lgl->out, "%s%s"
" seconds         irredundant          redundant clauses       "
"height      sat         jl' fast slow fast/\n",
    lgl->prefix, !lgl->tid ? "  " : "");
  fprintf (lgl->out, "%s%s"
"         variables clauses conflicts large ternary binary "
"glue        MB         it\"      glue glue  slow\n",
    lgl->prefix, !lgl->tid ? "  " : "");
  fprintf (lgl->out, "%s\n", lgl->prefix);
  fflush (lgl->out);
  if (lgl->cbs && lgl->cbs->msglock.unlock)
...
static double lglitavg (LGL * lgl) {
  return lgl->stats->its.avg / (double)(1ull<<32);
}
#endif

static double lglitder (LGL * lgl) {
  return lgl->stats->its.der / (double)(1ull<<32);
}

static double lgljlevelder (LGL * lgl) {
  return lgl->stats->jlevel.der / (double)(1ll<<16);
}

static double lglrelavgrealglue (LGL * lgl) {
  return lglpcnt (lgl->stats->avgrealglue, lgl->stats->jlevel.avg);
}
...
static void lglrep (LGL * lgl, int level, char type) {
  ...
  if ((level > 0 && lgl->repforcehead) || !lgl->repcntdown--)
    lglrephead (lgl);
  lglprt (lgl, level,
    "%c %6.1f %7d %8d %9lld %7d %6d %5d %4.0f %5.0f %4.0f"
    " %3.0f%% %6.0f %4.0f"
    " %4.0f %4.0f %4.0f%%"
    ,
    type,
    ...
    lglheight (lgl),
    lglmb (lgl),
    lglrelavgrealglue (lgl),
    lglitder (lgl),
    lgljlevelder (lgl),
    lglgluefast (lgl),
    lglglueslow (lgl),
    lglpcnt (lgl->stats->avgrealgluefast, lgl->stats->avgrealglueslow));
  lgl->stats->reported++;
}
...
static int lglgensatlimok (LGL * lgl, const char * type,
                           int lo, int hi,
			   int default_if_not_ready_yet) {
  uint64_t avglue, avgheight, hilim, lolim;
  assert (0 <= lo);
  assert (0 <= hi);
  if (lgl->stats->confs < lgl->opts->satlimdelay.val)
    return default_if_not_ready_yet;
  avglue = lgl->stats->avgrealglue;
  LOG (5, "average running real glue %.2f", avglue/(double)(1ull<<32));
  avgheight = lgl->stats->jlevel.avg;
  LOG (5, "average running jump level %.2f", avgheight/(double)(1ull<<32));
  lolim = lo * (avgheight/100);
  if (avglue < lolim) {
    LOG (4,
      "glue %.2f < %.2f = height %.2f * %d/100 lower %s limit",
      avglue/(double)(1ull<<32),
      lolim/(double)(1ull<<32),
      avgheight/(double)(1ull<<32), lo,
      type);
    return 0;
  }
  hilim = hi * (avgheight/100);
  if (avglue > hilim) {
    LOG (4,
      "glue %.2f > %.2f = height %.2f * %d/100 upper %s limit",
      avglue/(double)(1ull<<32),
      hilim/(double)(1ull<<32),
      avgheight/(double)(1ull<<32), hi,
      type);
    return 0;
  }
  assert (lolim <= avglue), assert (avglue <= hilim);
  LOG (4,
    "glue %.2f in [%.2f,%.2f] = height %.2f * [%d,%d]/100 %s limits",
    avglue/(double)(1ull<<32),
    lolim/(double)(1ull<<32), hilim/(double)(1ull<<32),
    avgheight/(double)(1ull<<32), lo, hi,
    type);
  return 1;
}

static int lglnorestartsatlimok (LGL * lgl) {
  return lglgensatlimok (lgl, "no restart saturating",
                         lgl->opts->norestartsatlimlo.val, 100, 0);
                              
}

static int lglredclsatlimok (LGL * lgl) {
  return lglgensatlimok (lgl, "clause reduction",
                         lgl->opts->redclsatlimlo.val,
			 lgl->opts->redclsatlimhi.val, 1);
}
...
static int64_t lglubyrec (LGL * lgl, unsigned i) {
  int64_t res = 0, s = 0;
  unsigned k;
...
static int64_t lglinout (LGL * lgl, int c, int relincpcnt) {
  ...
  while (c-- > 0) {
    ...
    if (i > (int64_t) INT_MAX) { i = INT_MAX; break; }
    if (i < o) continue;
    i = 1;
    o = ((100 + relincpcnt)*o + 99)/100;
    if (o > (int64_t) INT_MAX) o = INT_MAX;
  }
...
static int lglcmpbump (LGL * lgl, int a, int b) {
  return lgl->opts->bumpsort.val * (lgltrail (lgl, b) - lgltrail (lgl, a));
}
...
static void lglbumpseenlits (LGL * lgl) {
  ...
  if (lgl->opts->bumpsort.val)
    SORT (int, lgl->seen.start, lglcntstk (&lgl->seen), LGLCMPBUMP);
  for (p = lgl->seen.start; p < lgl->seen.top; p++) lglbumpdlit (lgl, p[0]);
  lglstop (lgl);
}
...
static void lglredcls (LGL * lgl, int uip, int glue, int * jlevelptr) {
  ...
  if (!lglredclsatlimok (lgl)) return;
...
RESTART:
  ...
  if (lgl->opts->bumpseen.val) lglbumpseenlits (lgl);
  ...
  if (!lgl->simp) {
    lgl->stats->avgrealglue -= lgl->stats->avgrealglue >> 12;
    lgl->stats->avgrealglue += ((uint64_t) realglue) << 20;

    lgl->stats->avgrealglueslow -= lgl->stats->avgrealglueslow >> 16;
    lgl->stats->avgrealglueslow += ((uint64_t) realglue) << 16;

    lgl->stats->avgrealgluefast -= lgl->stats->avgrealgluefast >> 8;
    lgl->stats->avgrealgluefast += ((uint64_t) realglue) << 24;

    lgl->stats->jlevel.avg -= lgl->stats->jlevel.avg >> 12;
    lgl->stats->jlevel.avg += ((uint64_t) jlevel) << 20;

    lgl->stats->jlevel.der -= lgl->stats->jlevel.der >> 16;
    lgl->stats->jlevel.der +=
      (((int64_t) lgl->stats->jlevel.avg) -
		  lgl->stats->jlevel.last) >> (32 - 16);
    lgl->stats->jlevel.last = lgl->stats->jlevel.avg;
  }
...
DONE:
  ...
  last = lgl->stats->its.avg;
  lgl->stats->its.avg -= lgl->stats->its.avg >> 16;
  ...
  if (len == 1) {
    ...
    assert (delta >= 0);
    lgl->stats->its.avg += ((uint64_t) delta) << 16;;
  }
  LOG (1, "iteration interval average %.0f", lglitavg (lgl));

  delta = ((int64_t) lgl->stats->its.avg) - last;
  lgl->stats->its.der -= lgl->stats->its.der >> 10;
  lgl->stats->its.der += delta;
  LOG (1, "iteration interval size derivation %.0f", lglitder (lgl));
...
static void lglincrestartlfixed (LGL * lgl, int skip) {
  int delta = lgl->opts->restartint.val;
  LOG (2, "fixed restart delta %d", delta);
  lgl->limits->restart.confs = lgl->stats->confs + delta;
  lglrep (lgl, 2, skip ? 'N' :'R');
}

static void lglincrestartlglc (LGL * lgl, int skip) {
  int64_t delta = lgl->opts->restartglcint.val;
  int pen = 0;
  if (lgl->opts->restartpen.val) {
    Stats * s = lgl->stats;
    if (s->confs > 500000)
    if (!s->its.count || s->confs/s->its.count > 100000) pen++;
    if (s->confs > 50000)
    if (!s->bins || s->confs/s->bins > 10000) pen++;
    if (s->confs > 5000)
    if (!s->trns || s->confs/s->trns > 1000) pen++;
    if (s->confs > 50000)
    if ((s->avgrealglue >> 32) > 20) pen++;
  }
  delta <<= pen;
  LOG (2, "glucose style fixed restart delta %d penalty %d", delta, pen);
  lgl->limits->restart.confs = lgl->stats->confs + delta;
  lglrep (lgl, 2, skip ? 'N' :'R');
}

static void lglincrestartaux (LGL * lgl, int skip) {
  int64_t delta = lgl->opts->restartint.val;
  int count;
  if (lgl->opts->restart.val == 2) {
    count = ++lgl->limits->restart.luby;
    delta *= lgluby (lgl, count);
    LOG (2, "luby restart delta %lld", (LGLL) delta);
  } else {
    assert (lgl->opts->restart.val == 3);
    count = ++lgl->limits->restart.inout;
    delta *= lglinout (lgl, count, lgl->opts->rstinoutinc.val);
    LOG (2, "inner/outer restart delta %lld", (LGLL) delta);
  }
  if (delta > (int64_t) INT_MAX) delta = INT_MAX;
  lgl->limits->restart.confs = lgl->stats->confs + delta;
...
#if 0
static int64_t lgladdsat64 (int64_t a, int64_t b) {
  assert (a >= 0), assert (b >= 0);
  if (LLMAX - b < a) return LLMAX;
  return a + b;
}

static int64_t lglmulsat64 (int64_t a, int64_t b) {
  assert (a >= 0), assert (b >= 0);
  if (!b) return 0;
  if (LLMAX / b < a) return LLMAX;
  return a * b;
...
static void lglincrestartl (LGL * lgl, int skip) {
  switch (lgl->opts->restart.val) {
    case 1: lglincrestartlfixed (lgl, skip); break;
    case 2: case 3: lglincrestartaux (lgl, skip); break;
    case 4: lglincrestartlglc (lgl, skip); break;
    default: break;
  }
}
...
static int lglnextdecision (LGL * lgl, int updatestats) {
  ...
  if (!res) {
	...
	if (!lglisfree (lgl, res)) {
      ...
      lgl->queue.next--;
      ASSERT (!lgl->opts->queuedfs.val || lgl->queue.next >= 0);
      if (lgl->queue.next < 0)
	lgl->queue.next = lglcntstk (&lgl->queue.stk) - 1;
      ASSERT (lgl->opts->queuedfs.val || lgl->queue.next != start);
    }
    lglstop (lgl);
...
static int lglreusetrail (LGL * lgl) {
  ...
  if (res)
    ...
    lglprt (lgl, 3, "[reuse-trail] can not reuse any level");
  return res;
}
...
static void lglrestart (LGL * lgl) {
  ...
  skip = 0;
  if (lglnorestartsatlimok (lgl))
    lgl->stats->restarts.skipped.saturating++, skip = 1;
  if (skip) {
    lgl->stats->restarts.skipped.count++;
    LOG (1, "skipping restart");
  } else {
    LOG (1, "restarting not saturating");
    if (!lgl->opts->reusetrail.val || lgl->opts->restart.val < 2) level = 0;
...
static void lglsetprbasiclim (LGL * lgl) {
  ...
  if (lgl->opts->prbasicrtc.val || lgl->opts->prbrtc.val) {
    ...
    irrlim = 8*lgl->stats->irr.clauses.cur;
    irrlim >>= lgl->limits->simp.pen;
    if (lgl->opts->irrlim.val && limit < irrlim) {
      limit = irrlim;
      lglprt (lgl, 1,
...
ALA:
  while (!res && nextala < lglcntstk (&lgl->seen)) {
	...
	for (q = d; (other = *q); q++) {
	  if (other == lit) continue;
	  if (lglsignedmarked (lgl, -other)) break;
          INCSTEPS (cce.steps);				// TODO since it is bad
	  if (lglsignedmarked (lgl, other)) continue;
	  if (unit) break;
	  unit = -other;
	}
...
static void lglseternreslim (LGL * lgl) {
  ...
  if (lgl->opts->ternresrtc.val) {
    ...
    limit >>= (pen = lgl->limits->ternres.pen + lglszpen (lgl));
	...
	lgl->stats->ternres.count, (LGLL) limit, lgl->stats->irr.clauses.cur);
    } else
      lglprt (lgl, 1, "[ternres-%d] limit %lld with penalty %d = %d + %d",
	lgl->stats->ternres.count, (LGLL) limit,
	pen, lgl->limits->ternres.pen, lglszpen (lgl));
...
static void lglquatres (LGL * lgl) {
  ...
  int glue, maxglue = lglscaleglue (lgl, 4), pivot, trn, a, b, redi, redj, red;
  const int * p, * c, * d;
  int assigned;
  POSLIDX * pls;
  Stk lits, clauses, *s;
  ...
  if (maxglue >= MAXGLUE) maxglue = MAXGLUE - 1;	// TODO not reachable?
  ...
  lglprt (lgl, 1,
    "[quatres-%d] added %d ternary resolvents",
    lgl->stats->quatres.count, trn);
  LGLUPDPEN (quatres, trn);
  assert (lgl->simp), assert (lgl->quatres), assert (lgl->notrim);
...
static void lglsetunhdlim (LGL * lgl) {
  ...
  irrlim = lgl->stats->irr.clauses.cur/16;
  irrlim >>= lgl->limits->simp.pen;
  if (lgl->opts->irrlim.val && limit < irrlim) {
    limit = irrlim;
    lglprt (lgl, 1,
...
static int lglrestarting (LGL * lgl) {
  ...
  if (lgl->opts->restart.val == 4) {
    uint64_t fast = lgl->stats->avgrealgluefast;
    uint64_t slow = lgl->stats->avgrealglueslow;
    fast /= lgl->opts->restartglclim.val;
    slow /= 100;
    if (fast < slow) {
      lgl->limits->restart.confs =
        lgl->stats->confs + lgl->opts->restartglcint.val;
      lglrep (lgl, 3, 'L');
      return 0;
    }
  }
...
static void lglsetsweeplim (LGL * lgl) {
  ...
  if (lgl->opts->sweeprtc.val > 1) {
    ...
    limit >>= (pen = lgl->limits->sweep.pen + lglszpen (lgl));
    irrlim = 2*lgl->stats->irr.clauses.cur;
    irrlim >>= lgl->limits->simp.pen;
    if (lgl->opts->irrlim.val && limit < irrlim) {
      limit = irrlim;
      lglprt (lgl, 1,
	"[sweep-%d] limit %lld based on %d irredundant clauses",
	lgl->stats->sweep.count,
	(LGLL) limit, lgl->stats->irr.clauses.cur);
    } else
      lglprt (lgl, 1,
	...
	lgl->stats->sweep.count, (LGLL) limit,
	pen, lgl->limits->sweep.pen, lglszpen (lgl));
    lgl->limits->sweep.steps = lgl->stats->sweep.steps + limit;
  }
}
...
static int lglsimplimhit (LGL * lgl, int * forcedptr) {
  ...
  if (lgl->stats->confs < lgl->limits->simp.hard) {
    if (lgl->stats->jlevel.der < 0) return 0;
    if (lgl->stats->its.der < 0) return 0;
  }
  if (lgl->stats->confs >= lgl->limits->simp.confs) {
    lglprt (lgl, 1, "");
    lglprt (lgl, 1,
      "[simplification-%d] limit %lld conflicts (hard %lld) hit after %lld conflicts",
...
void lglstats (LGL * lgl) {
  ...
  lglprs (lgl,
    "otfs: str %lld, %d red %.0f%%, %d irr %.0f%%",
    sum,
    s->otfs.str.red, lglpcnt (s->otfs.str.red, sum),
    s->otfs.str.irr, lglpcnt (s->otfs.str.irr, sum));
  lglprs (lgl,
    "otfs: str %lld, %d bin %.0f%%, %d trn %.0f%%, %d lrg %.0f%%",
    sum,
    s->otfs.str.bin, lglpcnt (s->otfs.str.bin, sum),
    s->otfs.str.trn, lglpcnt (s->otfs.str.trn, sum),
    s->otfs.str.lrg, lglpcnt (s->otfs.str.lrg, sum));

  sum = s->otfs.sub.red + s->otfs.sub.irr;
  assert (sum == s->otfs.sub.bin + s->otfs.sub.trn + s->otfs.sub.lrg);
  lglprs (lgl,
    "otfs: sub %lld, %d red %.0f%%, %d irr %.0f%%",
    sum,
    s->otfs.sub.red, lglpcnt (s->otfs.sub.red, sum),
    s->otfs.sub.irr, lglpcnt (s->otfs.sub.irr, sum));
  lglprs (lgl,
    "otfs: sub %lld, %d bin %.0f%%, %d trn %.0f%%, %d lrg %.0f%%",
    sum,
    s->otfs.sub.bin, lglpcnt (s->otfs.sub.bin, sum),
    s->otfs.sub.trn, lglpcnt (s->otfs.sub.trn, sum),
    s->otfs.sub.lrg, lglpcnt (s->otfs.sub.lrg, sum));
  ...
  lglprs (lgl,
    "quat: %d count, %lld trn, %lld sub",
    s->quatres.count, (LGLL) s->quatres.trn, (LGLL) s->quatres.sub);
  ...
  lglprs (lgl,
    "rsts: %lld restarts %.0f%%, %lld skipped %.0f%%",
    (LGLL) s->restarts.count,
      lglpcnt (s->restarts.count, sum),
    (LGLL) s->restarts.skipped.count,
      lglpcnt (s->restarts.skipped.count, sum));
  lglprs (lgl,
    "rsts: %lld saturating %.0f%%, %lld agile %.0f%%, %lld reuse %.0f%%",
    (LGLL) s->restarts.skipped.saturating,
      lglpcnt (s->restarts.skipped.saturating, s->restarts.skipped.count),
    (LGLL) s->restarts.skipped.agile,
      lglpcnt (s->restarts.skipped.agile, s->restarts.skipped.count),
    (LGLL) s->restarts.skipped.reuse,
      lglpcnt (s->restarts.skipped.reuse, s->restarts.skipped.count));
  lglprs (lgl,
    "rsts: %lld kept %.1f%% average %.1f%%",
  ...
  lglprs (lgl,
    ...
    s->setscincf.max/10.0, s->setscincf.min/10.0);
